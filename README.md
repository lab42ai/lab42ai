# Lab42AI

AI security research & engineering. We break AI to make it safer.

**What we do:**
- AI system security testing and red-teaming
- AI system vulnerability research
- Open-source tooling for AI security
- Tools and platforms for AI security
- AI infrastructure for privacy

**What we're building:**
- [Minion](https://github.com/lab42ai/minion) — a security-first autonomous AI agent
- [Zeroshot]([https://0eroshot.com](https://github.com/lab42ai/zeroshot) — ZeroShot is a terminal-first red-team engine for text-based AI agents

**Read our writings:** [Substack](https://lab42ai.substack.com)


